...
from Manager.__main__ import (IMPORTED, HELPABLE, MIGRATEABLE, STATS, USER_INFO, DATA_IMPORT, DATA_EXPORT, CHAT_SETTINGS,
                             USER_SETTINGS)
from Manager.modules.helper_funcs.chat_status import sudo_plus, dev_plus
...
def load(bot: Bot, update: Update):
    ...
    try:
        imported_module = importlib.import_module("Manager.modules." + text)
    except:
...
def listmodules(bot: Bot, update: Update):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("Manager.modules.", 1)[1]
